...
Write-Host "  MediWatt - independent horizontal scaling demo" -ForegroundColor Cyan
Write-Host "======================================================" -ForegroundColor Cyan

Write-Host ""
Write-Host "This is what we have right now:" -ForegroundColor Cyan
kubectl get deployments -n mediwatt

Pause-Demo "Now I scale ONLY the optimizer to 6 replicas."

kubectl scale deployment optimizer --replicas=6 -n mediwatt
Start-Sleep -Seconds 3
kubectl rollout status deployment/optimizer -n mediwatt --timeout=120s
...
kubectl get pods -n mediwatt -l app=optimizer -o wide
...
$victim = (kubectl get pods -n mediwatt -l app=optimizer -o jsonpath="{.items[0].metadata.name}")
Write-Host "Deleting pod: $victim" -ForegroundColor Cyan
kubectl delete pod $victim -n mediwatt
Start-Sleep -Seconds 5
Write-Host ""
Write-Host "Kubernetes immediately created a replacement:" -ForegroundColor Cyan
kubectl get pods -n mediwatt -l app=optimizer

Pause-Demo "Scaling the optimizer back down to 2."

kubectl scale deployment optimizer --replicas=2 -n mediwatt
Start-Sleep -Seconds 3
kubectl get deployments -n mediwatt
